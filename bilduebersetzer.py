...
def clean_text(text: str) -> str:
    """Entfernt doppelte Zeilen, bereinigt Text und ersetzt escaped \n."""
    if not text:
        return ""
    # Escaped \n (als Text) in echte Zeilenumbrüche umwandeln
    text = text.replace("\\n", "\n")
    lines = text.split("\n")
    seen = []
    for line in lines:
    ...
    async def uebersetze_bild(self, ctx):
        ...
        async def process_single(url: str, index: int) -> discord.Embed | None:
            try:
                ...
                # Bildübersetzer zeigt immer alle 4 Sprachen — unabhängig von globalen Einstellungen
