...
# Kanal-ID wo der Bot seinen Start-Status postet
# Trage hier die ID deines Bot-Log oder Admin-Kanals ein!
...
async def on_ready():
    ...
    # Status-Nachricht in Discord posten
...
async def cmd_help(ctx):
    ...
    # ── Übersetzer ──
    embed.add_field(
        name="🌐 Übersetzer / Traducteur",
        value=(
            "`!translate on/off` – 🇩🇪 An/Aus  •  🇫🇷 Activer/Désactiver\n"
            "`!translate status` – 🇩🇪 Status  •  🇫🇷 Statut\n"
            "`!ai [Frage/Question]` – 🇩🇪 KI fragen  •  🇫🇷 Poser une question"
        ),
        inline=False
    )
    ...
    # ── Koordinaten ──
    embed.add_field(
        name="📍 Koordinaten / Coordonnées",
        value=(
            "`!koordinaten` / `!coordonnees` – 🇩🇪 Alle anzeigen  •  🇫🇷 Tout afficher\n"
            "`!koordinaten add NAME R X Y` / `!coordonnees ajouter` – 🇩🇪 Hinzufügen  •  🇫🇷 Ajouter\n"
            "`!koordinaten delete NAME` / `!coordonnees supprimer` – 🇩🇪 Löschen  •  🇫🇷 Supprimer\n"
            "🔐 *Nur / Seulement: Administrator, R5, R4*"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="⏱️ Timer / Rappel",
        value=(
            "`!timer DAUER EVENT` / `!rappel DURÉE EVENT` – 🇩🇪 Timer setzen  •  🇫🇷 Définir\n"
            "`!timer list` / `!rappel list` – 🇩🇪 Aktive Timer  •  🇫🇷 Minuteurs actifs\n"
            "`!timer delete NAME` / `!rappel supprimer NAME` – 🇩🇪 Löschen  •  🇫🇷 Supprimer\n"
